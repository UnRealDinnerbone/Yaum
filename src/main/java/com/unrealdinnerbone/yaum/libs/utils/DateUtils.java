...
public class DateUtils {
    ...
    public static void checkDates() {
        if(isToday(1, 1) || isToday(12, 31)) {
            Yaum.getInstance().getLogHelper().info("Happy New Year");
        }else if(isToday(4, 1))        {
            Yaum.getInstance().getLogHelper().info("Happy April Fools Day");
        }else if(isToday(10, 31)) {
            Yaum.getInstance().getLogHelper().info("Happy Halloween");
        }else if(isToday(12, 24) || isToday(12, 25)) {
            Yaum.getInstance().getLogHelper().info("Merry Christmas");
        }else {
            Yaum.getInstance().getLogHelper().info(":( Today is not a Holiday");
        }
